...
function ManageInventory() {
  ...
  const fetchItems = async () => {
    try {
      const response = await axios.get('https://lostandfoundback-184f1a940482.herokuapp.com/api/Items');
  ...
  const handleAddItem = async (event) => {
    ...
    try {
      await axios.post('https://lostandfoundback-184f1a940482.herokuapp.com/api/Items', newItem);
      fetchItems();
  ...
  const handleSaveEdit = async (event) => {
    event.preventDefault();
    try {
      await axios.put(`https://lostandfoundback-184f1a940482.herokuapp.com/api/Items/${editingItem.item_id}`, newItem);
      fetchItems();
      setEditingItem(null);
      setNewItem({ description: '', date_found: '', location_found: '', category: '', photo_url: '' });
    } catch (error) {
  ...
  const handleRemoveItem = async (id) => {
    try {
      await axios.delete(`https://lostandfoundback-184f1a940482.herokuapp.com/api/Items/${id}`);
      fetchItems();
    } catch (error) {
